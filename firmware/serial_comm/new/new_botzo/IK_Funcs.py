...
def BR_legIK(x,y,z):
  ...
  coxa_angle = (np.arctan2(y,z) + np.arctan2(D,coxa)) # without' 180 -'
  femur_angle = deg2rad(90) - (shoulder_angle + adjustment)
  tibia_angle = np.pi - knee_angle + femur_angle + adjustment
...
def BL_legIK(x,y,z):
  ...
  coxa_angle = deg2rad(175) - ((deg2rad(180) - (np.arctan2(y,z) + np.arctan2(D,coxa))) - (2 * ((deg2rad(180) - (np.arctan2(y,z) + np.arctan2(D,coxa)))-deg2rad(90))))
  femur_angle = deg2rad(90) - (shoulder_angle + adjustment)
